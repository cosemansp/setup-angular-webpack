...
export class FakeBackend extends MockBackend {
  ...
  public flush() {
    if (this._connections.length === 0) {
      throw new AssertionError('No connections to flush')
    }
  ...
  public verifyNoPendingExpectations() {
    ...
    if (notVerifiedExpectations.length > 0) {
      throw new AssertionError(`Pending expectations found: ${notVerifiedExpectations.length}`)
    }
  }
  ...
  public verifyNoPendingRequests() {
    ...
    if (notVerifiedConnections.length > 0) {
      throw new AssertionError(`Pending connections found: ${notVerifiedConnections.length}`)
    }
  }
  ...
  private _verifyExpectation(order: number) {
    if (!this._expectations[order]) {
      throw new AssertionError('No expectation to fulfill')
    }
